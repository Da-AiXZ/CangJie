...
//  Q2 决策：从 ChapterContentPanel 重命名为 ChapterEditorView，恢复语义。
//

import SwiftUI

/// 章节内容编辑视图（Q2 重命名：ChapterContentPanel → ChapterEditorView）
struct ChapterEditorView: View {
    ...
    private var bottomStatusBar: some View {
        HStack(spacing: Theme.Spacing.md) {
            ...
            // P0-3: 在独立页编辑入口，NavigationLink push 到 ChapterEditView
            if let novelId = novelStore.currentNovel?.id {
                NavigationLink {
                    ChapterEditView(novelId: novelId, chapterNumber: chapter.number)
                } label: {
                    Label("独立页编辑", systemImage: "rectangle.expand.vertical")
                        .font(.system(size: 12))
                }
                .buttonStyle(.bordered)
            }
