...
TrollStore build run `29556797484` compiled the Release app successfully but failed before signing because the processed app `Info.plist` omitted `CangJieGitCommit` even though Xcode received the custom build setting. The packaging script now stamps the exact 12-character HEAD identity into the built plist atomically after compilation and before any signing, rejects malformed commits, unexpected pre-existing identities, wrong numeric build numbers, symlinks, and invalid plists, then reopens and verifies the stamped value. The existing package verifier still independently checks bundle ID, minimum OS, device family, executable name, build number, and commit.
