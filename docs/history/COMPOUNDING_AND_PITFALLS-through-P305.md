...
P-270 subtree evidence: exact commit `5bdf141` structurally removed the covered composer and advanced the first failure from its absence assertion to the next covered child, `result-drawer-toggle` at line 70. When successive children of the same covered surface escape composite accessibility modifiers, do not add one-off gates to every leaf. Gate the complete covered conversation control subtree while retaining all durable state and runtime work in `AppViewModel`, then verify exact restoration after dismissal.

P-270 activity-bar sibling evidence: on exact commit `9dd360492e67805b811daa51bda8532d315ef8d9`, structural removal of the covered `conversation` view advanced the first main App UI failure to `CangJieSmokeUITests.swift:71`, where `activity-bar-conversation` remained queryable after the landscape independent page opened. The existing dynamic hidden modifier on the composite `activityBar` was not sufficient in the observed XCUITest hierarchy. The follow-up keeps the activity-bar body unchanged in `activityBarContent` and structurally constructs the public `activityBar` only for the active conversation destination. Accept only after the exact replacement iPadOS CI proves absence while the modal is open and restoration after dismissal; do not weaken the assertion or add one-off leaf gates.

P-270 conversation-rail sibling evidence: exact replacement commit `0f50983` removed the covered activity-bar query from the first failure, advancing the iPadOS UI failure from line 71 to `CangJieSmokeUITests.swift:72`, where `landscape-conversation-rail` remained queryable. This confirms the same dynamic composite-hidden limitation applies to the adjacent conversation rail. The next slice structurally constructs the rail only for the active conversation destination while keeping its original body and navigation state unchanged; accept only after exact CI verifies covered absence and post-dismissal restoration.

P-270 portrait top-bar evidence: exact replacement commit `fbad49a` removed the covered conversation rail query from the first failure, advancing the iPadOS UI failure to `CangJieSmokeUITests.swift:101`, where `portrait-navigation-open` remained queryable under the portrait navigation modal. The next slice structurally constructs the top bar only when its surface is active, preserving the original focus controls and navigation action while removing the covered subtree instead of relying on a dynamic composite hidden modifier.

## P-271 Portrait focus visibility must be structural, not dynamic hiding
...
Acceptance evidence: exact commit `f93d43b` passed Core and iPadOS CI, including an enabled canary-control assertion after bounded lazy-List reveal. Paired build run `29787116654` then produced and independently verified run-31 Main and Probe IPAs. The remaining fail-closed state is now solely the intended TrollStore physical-device isolation and single-overwrite activation gate.

## P-278 One overwrite and one process relaunch are different acceptance events

A TrollStore overwrite can be performed exactly once while the first subsequent launch still fails closed and asks for the old process to be terminated. A force-quit/relaunch is not a second overwrite, but it also does not by itself prove activation. Accept the candidate only after the relaunched process reports the exact expected version, build, commit and Candidate Set, the running executable matches the installed bundle, the paired Probe proves Keychain isolation, and all required product smoke checks pass. If the first-launch warning text was not captured, record that limitation and do not invent its root cause. Future candidates must retain fail-closed identity diagnostics and capture the exact warning when it recurs; never normalize a second overwrite or uninstall as the recovery path.
...
Acceptance evidence: exact repair commit `cb9b4ebd536ef6dd02c6448f179c4c1f1f145841` passed Core CI `29916742888` and iPadOS CI `29916742840`. Paired workflow `29918215122` produced version `1.0` / build `33001` Main and Probe IPAs with Candidate Set ID `06d268b604c8eefee21814bef8b41e0bdd46226ff52b18ed7ad9508d6b6e19a7`; macOS strict signing verification and independent hashes, arm64 Mach-O, `LC_CODE_SIGNATURE` and distinct-Keychain-group checks passed. Device acceptance remains pending and must not be inferred from automation.
