...
Exact commit `117000e3d22753fbd9dcc5e1cd38d8c2bb5f6cf4` passed Core CI `29913815917` but iPadOS CI `29913815925` stopped during App test compilation before executing tests. The complete failed-step log contains one causal compiler error: `ModelConnectionRecoveryViewModelTests.swift:124` supplied `isolationCanaryRepository` before the earlier-declared `modelCredentialRepository` argument. The local repair only restores the initializer's declaration order; Swift parsing, a repository-wide reversed-order scan and `git diff --check` pass. Replacement exact-SHA Core/iPadOS CI is required.

Replacement commit `d8ba299f2e5b42ad6e001e758799662f1420b3bd` passed Core CI `29915021196` and advanced iPadOS CI `29915021352` through compilation and all suites. The complete log grouped the remaining failures into three causes rather than eleven independent assertions: optional equality treated absent pending intent and an unbound Conversation as a match, disabling the ordinary composer and returning the wrong connection status; one Conversation-switch test expected an ID before the first turn atomically created it; and three hanging test doubles used `Task.sleep(UInt64.max)`, which is not a portable suspension value on Apple and let the Custom authentication test surface `invalidResponse` instead of deadline cancellation. The local repair unwraps pending intent before scope comparison, moves the new-Conversation ID assertion after the first S2 turn, and uses a valid cancellable 60-second suspension in all three test doubles. Four Swift files parse, the root-cause pattern scan and `git diff --check` pass; replacement exact-SHA Core/iPadOS CI is required.

Exact repair commit `cb9b4ebd536ef6dd02c6448f179c4c1f1f145841` passed Core CI `29916742888` and iPadOS CI `29916742840`. Paired Candidate Set workflow `29918215122` then passed for version `1.0`, build `33001`, Candidate Set ID `06d268b604c8eefee21814bef8b41e0bdd46226ff52b18ed7ad9508d6b6e19a7`. Main IPA SHA-256 is `1ab74cc072cda4c510ca7761225f9a2391df6c2c075f66534087691cd4777530`; Probe IPA SHA-256 is `70ce44ec5e4786a7b1829ca8d8a381d142ee22e2ebb0046e93a531fcae2ca084`. The macOS workflow passed strict signing verification. Independent Windows inspection revalidated manifest/checksum agreement, version/build/bundle metadata, arm64 Mach-O payloads, non-empty `LC_CODE_SIGNATURE` commands and distinct Main/Probe Keychain groups. Acceptance remains fail closed pending exact-pair TrollStore installation, Main canary and Probe isolation checks on the target device.
