# 仓颉 Agent Harness 设计参考

- 状态：`NON-AUTHORITATIVE DESIGN REFERENCE`
- 决策日期：2026-07-18
- 适用范围：Context、Prompt、Agent Loop、Typed Tools、任务恢复、多 Agent、治理与可观测性
- 权威边界：`docs/adr/0004-host-control-and-model-trust-boundary.md` 冻结宿主控制与模型信任边界；本文只提供按需加载的设计分解，不能覆盖 ADR，也不能证明实现状态。
- 关联文档：`docs/IMPLEMENTATION_PLAN.md`、`docs/PROJECT_CONTROL_CENTER.md`、`docs/PRODUCT_EXPERIENCE_BLUEPRINT.md`、`docs/MILESTONE_VISUAL_ACCEPTANCE.md`

> **CJ-AH-001 - DESIGN REFERENCE - 2026-07-18**
>
> 仓颉宿主掌握主循环、真实状态、权限、预算、事务、恢复和完成判定；模型只负责判断下一步并请求工具，不能自行判定软件操作成功。
>
> 实现、验证和验收状态只读取 `docs/PROJECT_CONTROL_CENTER.md`。
...
**权威边界：本文是设计参考；宿主控制与模型信任边界以 ADR-0004 为准，当前实现和验收状态只以 `PROJECT_CONTROL_CENTER.md` 为准。**
