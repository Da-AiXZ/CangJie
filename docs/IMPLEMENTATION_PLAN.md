...
CangJie uses **S0–S6 user-visible product stages** and **H0–H5 sequential Agent Harness gates**. The current real milestone is **S1 Agent 驾驶舱定调与重构**. S0 means only that the technical-feasibility baseline was completed. Historical candidate-hardening M1 labels and Builds 26–28 remain engineering-prototype and hardening evidence; they are not the current complete-product milestone, and Build 28 is not accepted. Green CI, a static interface, documentation, or completed code alone cannot pass a product stage.
